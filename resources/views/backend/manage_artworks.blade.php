                                        ...
                                        <table id="datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <a href="{{url('change_artwork_status')}}/{{$artwork->id}}/{{$artwork->is_publised}}/manage_artworks/{{$artwork->gallery_user_id}}" class="btn btn-danger btn-link btn-sm change_artwork_status" title="@if($artwork->is_publised == 'publish') Click to Un-Publish @else Click to Publish @endif"><i class="fa fa-power-off"></i></a>
                                                        <a href="{{url('change_top_status')}}/{{$artwork->id}}/{{$artwork->top}}/manage_artworks/{{$artwork->gallery_user_id}}" class="btn btn-danger btn-link btn-sm change_top_status" title="@if($artwork->top == 'yes') Remove From Top @else Add to Top @endif">@if($artwork->top == 'yes') <i class="fa fa-minus-square"></i> @else <i class="fa fa-plus-square"></i> @endif</a>
                                                        <a href="{{url('change_trending_status')}}/{{$artwork->id}}/{{$artwork->trending}}/manage_artworks/{{$artwork->gallery_user_id}}" class="btn btn-danger btn-link btn-sm change_trending_status" title="@if($artwork->trending == 'yes') Remove From Trending @else Add to Trending @endif">@if($artwork->trending == 'yes') <i class="fa fa-minus-square"></i> @else <i class="fa fa-plus-square"></i> @endif</a>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
